-- Rate limit: ten sam email nie może wysłać wiadomości w ciągu 30 minut
...
begin
  if not public.check_contact_rate_limit(new.email) then
    raise exception 'Ten adres email wysłał już wiadomość w ostatnich 30 minutach. Spróbuj ponownie później.';
  end if;
  return new;
end;
$$;
